...
int main() {
	...
	vector<std::string> do_1_hashes, do_2_hashes, do_3_hashes, do_4_hashes;
	...
	//cout << "size of vectors  " << do_1_hashes.size() << "  " <<  do_2_hashes.size() << "  " <<  do_3_hashes.size() << " " << do_4_hashes.size() << endl;
	...
	//cout << "size of set intersection  " << intersect1234.size() << endl;
	...
	int cardinality = 0;
    for (its = intersect1234.begin(); its != intersect1234.end(); ++its)
    {
	...
	for(int i = 0; i < number_of_ciphertexts; i++)
	{
		...
		count_sum_cipher = (do_one_count_cipher + do_two_count_cipher + do_three_count_cipher + do_four_count_cipher - c_th) * c_rand;
