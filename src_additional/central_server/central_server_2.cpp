...
int main() {
    ...
    */
	...
	//cout << "size of vectors  " << do_1_hashes.size() << "  " <<  do_2_hashes.size() << endl; //"  " <<  do_3_hashes.size() << endl;
	...
	//cout << "size of set intersection  " << intersect12.size() << endl;
	...
	int cardinality = 0;
    for (its = intersect12.begin(); its != intersect12.end(); ++its)
    {
	...
	*/   
	...
	for(int i = 0; i < number_of_ciphertexts; i++)
	{
		...
		*/
		...
		count_sum_cipher = (do_one_count_cipher + do_two_count_cipher - c_th) * c_rand;
