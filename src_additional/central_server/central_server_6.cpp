...
int main() {
	...
	std::string do_6_ip_str =  config_reader.Get("ip_and_port", "do_6_ip", "UNKNOWN");
	uint16 do_6_port =  config_reader.GetInteger("ip_and_port", "do_6_port", -1);
	uint16 cs_port_6 =  config_reader.GetInteger("ip_and_port", "cs_port_6", -1);
	...
	ip_address do_6_ip = GetHostByName(do_6_ip_str.c_str());
	//SendFile(do_6_ip, do_6_port, "central_server/config.ini");
	while(1)
	{
		if(SendFile(do_6_ip, do_6_port, "central_server/config.ini"))
			break;
	}
	ReceiveMessage(cs_port_6);
	...
	#pragma omp parallel sections num_threads(6)
    {
		...
		#pragma omp section
        {
	        ...
	        while(1)
	        {
		        if(ReceiveFile(cs_port_6, std::string("central_server/" + exp_code + "/6_hashes.txt").c_str()))
			     break;
	        }
	        //Comment: Send acknowledge for hahses
	        while(1)
	        {
		        if(SendMessage(do_6_ip, do_6_port, std::string("hashes_received_by_cs").c_str()))
			    break;
	        }
        }
	...
	vector<std::string> do_1_hashes, do_2_hashes, do_3_hashes, do_4_hashes, do_5_hashes, do_6_hashes;
	...
	#pragma omp parallel sections num_threads(6)
    {
		...
		#pragma omp section
        {
			//vector<std::string> do_6_hashes;
            ifstream hashes_of_data_6_stream("central_server/" + exp_code + "/6_hashes.txt");
	        std::string line6;
	        if (hashes_of_data_6_stream.is_open()) {

                while (!hashes_of_data_6_stream.eof() && getline(hashes_of_data_6_stream, line6)){
                    line6.erase(line6.find_last_not_of(" \n\r\t")+1);
			        do_6_hashes.push_back(line6);	 
                }
	...
	//cout << "size of vectors  " << do_1_hashes.size() << "  " <<  do_2_hashes.size() << "  " <<  do_3_hashes.size() << " " << do_4_hashes.size() << " " << do_5_hashes.size()
    //<< " " << do_5_hashes.size()	<< endl;
	...
	#pragma omp parallel sections num_threads(6)
    {
		...
		#pragma omp section
        {
			std::sort(do_6_hashes.begin(), do_6_hashes.end());
        }
	...
	vector<std::string> intersect123456;
    set_intersection(intersect12345.begin(),intersect12345.end(),do_6_hashes.begin(),do_6_hashes.end(),
                  std::back_inserter(intersect123456));
	
	//cout << "size of set intersection  " << intersect123456.size() << endl;
	...
	int cardinality = 0;
    for (its = intersect123456.begin(); its != intersect123456.end(); ++its)
    {
	...
	while(1)
	{
		if(SendFile(do_6_ip, do_6_port, std::string("central_server/" + exp_code + "/intersection_hashes.txt").c_str()))
			break;
	}
	ReceiveMessage(cs_port_6);
	...
	#pragma omp parallel sections num_threads(6)
    {
		...
		#pragma omp section
        {
	        ...
	        while(1)
	        {
		        if(ReceiveFile(cs_port_6, std::string("central_server/" + exp_code + "/ciphertext/6_cipher_file.encrypted").c_str()))
			     break;
	        }
	        //Comment: Send acknowledge for ciphertexts
	        while(1)
	        {
		        if(SendMessage(do_6_ip, do_6_port, std::string("ciphertext_received_by_cs").c_str()))
			    break;
	        }
	...
	ifstream read_counts_six("central_server/" + exp_code + "/ciphertext/6_cipher_file.encrypted");
	if(read_counts_six.is_open())
		cout<< "encrypted count from do 6 opened" << endl;
	else
		cout << "file opening error" << endl;
	...
	for(int i = 0; i < number_of_ciphertexts; i++)
	{
		...
		ciphertext_t do_six_count_cipher;
		//seek_position = (*its).second.at(2)*2*4*4096L;
		read_counts_six.seekg((unsigned long)seek_position, read_counts_six.beg);
		do_six_count_cipher.c0.deserialize_manually(read_counts_six);
		do_six_count_cipher.c1.deserialize_manually(read_counts_six);
		do_six_count_cipher.isnull= false;
		do_six_count_cipher.pk = &pk;
		...
		count_sum_cipher = (do_one_count_cipher + do_two_count_cipher + do_three_count_cipher + do_four_count_cipher + do_five_count_cipher + do_six_count_cipher - c_th) * c_rand;
	...
	while(1)
	{
		if(SendFile(do_6_ip, do_6_port, std::string("central_server/" + exp_code + "/ciphertext/result.encrypted").c_str()))
			break;
	}
	ReceiveMessage(cs_port_6);
